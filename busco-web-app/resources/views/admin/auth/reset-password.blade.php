{{-- View: admin/auth/reset-password.blade.php | Purpose: Admin authentication page template. --}}
...
<head>
    ...
    <title>Reset Password | BUSCO Admin</title>
    ...
    <style>
        ...
        .login-wrap { width: min(100%, 480px); background: #fff; border-radius: 18px; border: 1px solid #e3eadb; box-shadow: 0 20px 45px rgba(24, 54, 29, .1); padding: 24px; }
...
<body>
    <section class="login-wrap" aria-labelledby="admin-reset-title">
        @include('partials.flash-messages')

        <div class="login-kicker">Administrator Access</div>
        <h1 class="login-title" id="admin-reset-title">Reset Admin Password</h1>
        <p class="login-copy">
            Set a new password for your admin account. After a successful reset, you will be redirected to the admin login page.
        </p>
        ...
        <form method="POST" action="{{ route('admin.password.store') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-input" id="email" name="email" type="email" value="{{ old('email', $email) }}" required autocomplete="username" autofocus>
            </div>
